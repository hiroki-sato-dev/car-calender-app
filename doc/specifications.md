...
新規機能の要件・技術仕様・設計方針は `doc/feature/` 配下で機能単位に管理する。

| 機能 | ドキュメント |
|------|------------|
| マイカレンダー | [doc/feature/myCalendar/](feature/myCalendar/) |

各機能フォルダには以下の3ファイルを用意する:
...
- PWA 対応: ホーム画面追加でネイティブアプリ風に起動できる（`display: standalone`）
...
| `leaveCalendar(calendarId)` | `number` | `{ success } \| { error }` | カレンダーから退会（最後の1人なら Calendar・Event も削除） |
...
- `join` イベント: Bot がグループに参加した際、登録コードの送信を案内するメッセージを返信
- `message` イベント（`source.type === "group"`）: テキストが `登録コード: XXXXXX` 形式の場合:
  - 該当 `shareCode` のカレンダーに `lineGroupId` を保存
  - 成功時: 「〇〇と連携しました！」を返信
  - 失敗時（コード不一致）: 「登録コードが見つかりませんでした」を返信
  - それ以外のメッセージ: 何も返信しない
- `message` イベント（`source.type === "user"`）: 個別チャットへの案内メッセージを返信（グループへの追加を促す）

### LINE 連携のリセット

`Calendar.lineGroupId` を `null` に設定すると連携が解除される。Prisma Studio（`npx prisma studio`）で操作可能。解除後はカレンダー画面に LINE 設定案内が再表示される。

### 通知メッセージ仕様

- メッセージ形式: LINE Flex Message（Bubble）
- URL プレビューサムネを表示しないようにするため、テキストメッセージではなく Flex Message のボタンアクション（`uri`）でリンクを設置
- カレンダー名は動的に埋め込む（「車の予定が〜」ではなく「{カレンダー名}の予定が〜」）
- 時刻表示は `timeZone: "Asia/Tokyo"` を明示して JST で表示
- 予定登録時刻は EventModal（クライアント）で `new Date(...).toISOString()` を使い JST → UTC 変換してから保存

**Flex Message 構成**
- Body: カレンダー名・登録者・日時（曜日付き）・タイトル・メモ
- Footer: 「カレンダーを開く」ボタン（カレンダーページへの直リンク）
...
- ヘッダー: アプリ名 + ユーザー名（タップ → ドロップダウン: 「名前を変更」「ログアウト」）
- **名前変更モーダル**: 新しいユーザー名を入力して変更。重複チェックあり。変更後は JWT を再発行してヘッダー表示に即反映
- カレンダーリスト: 名前 + 共有コード + コピーボタン（2 秒後に元の表示に戻る）
- カレンダーカードタップ時: カード右端にスピナーを表示してページ遷移を待機
- ボタン: 「参加」「+ 作成」
- モーダル:
  - **作成モーダル**: カレンダー名入力 → 作成後に共有コード表示モーダルへ遷移
  - **参加モーダル**: 共有コード入力（大文字変換して照合）
  - **コード表示モーダル**: 生成された共有コードを大きく表示・コピーボタン（押すとボタンテキストが「コピーしました」に 2 秒間変化）

### カレンダー画面 (`/calendar/[id]`)

- ヘッダー: 戻るボタン + カレンダー名 + 「カレンダーを削除」ボタン（右端）+ メンバー一覧（カラードット + 名前）
- LINE 未連携時: LINE設定案内（Bot追加リンク・登録コードコピー）を表示。コピーボタン押下で画面下部にトーストを 2 秒間表示
- FullCalendar 月表示
- 日付クリック → DayModal

#### カレンダー削除

- 「カレンダーを削除」タップ → 確認モーダルを表示
- 確認後: `CalendarMember` レコードを削除（カレンダー自体は残る）
- 削除後のメンバー数が 0 人の場合: `Event` を全削除してから `Calendar` 自体も削除
- 削除完了後 `/calendars` にリダイレクト
...
- 当日の予定リスト（カラードット + タイトル + 時間帯 + 登録者）
  - 同日予定: `18:00 〜 19:00` 形式
  - 日跨ぎ予定: `3/22(土) 18:00 〜 3/23(日) 8:00` 形式（月/日(曜日)を付加）
...
- 閲覧モード: タイトル・登録者・日時・メモを表示。オーナーのみ編集・削除ボタンを表示
  - 日時: `4月10日(木) 13:00 〜 17:00` 形式。日跨ぎの場合は終了側にも日付(曜日)を表示
- 編集モード: フォームに既存値をセット。保存時に `updateEvent` を呼び出し
- 作成モード: 日付を DayModal から引き継いで初期値にセット
- 送信時は `new Date(`${date}T${time}`).toISOString()` でクライアントのローカル時刻（JST）をUTCに変換してからサーバーに送る（タイムゾーンずれ防止）
...
| ローディング表示 | 非同期処理中のボタン（作成・参加・登録・保存・削除）はスピナー（`Spinner` コンポーネント）+ テキストを表示。カレンダーカード遷移時はカード内にスピナーを表示 |
| iOS ズーム防止 | `input` の `font-size` を 16px 以上に固定（グローバル CSS）。`body` と `.calendar-wrapper` に `overflow-x: hidden` を設定し横幅オーバーフローによるズームアウトを防止 |
| PWA | `public/manifest.json` で `display: standalone` を設定。Safari「ホーム画面に追加」でアドレスバーなしのネイティブ風起動が可能。アイコン: `icon-192.png`, `icon-512.png`, `apple-touch-icon.png`（180×180）。ファビコン: `app/favicon.ico` |
...
| Phase 6 | UX 改善（PWA・ローディング・タップフィードバック・曜日表示・iOS ズーム対応） | 完了 |
